...
import com.emmisolutions.emmimanager.web.rest.client.model.user.sercret.question.response.*;
    ...
    */
    ...
    public ResponseEntity<SecretQuestionPage> getSecretQuestionWithResetToken(
       ...
       if(page != null) {
        	...
        	List<SecretQuestion> question = new ArrayList<>();
            for(UserClientSecretQuestionResponse response : page){
        		question.add(response.getSecretQuestion());
           	}
        	Page<SecretQuestion> emptyResponsePage = new PageImpl<>(question,pageRequest,0);
        	return new ResponseEntity<>(
