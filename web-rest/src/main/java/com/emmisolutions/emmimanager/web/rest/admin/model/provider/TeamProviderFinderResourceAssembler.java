...
public class TeamProviderFinderResourceAssembler implements ResourceAssembler<TeamProvider, TeamProviderResource> {
    ...
    @Resource
    ResourceAssembler<Provider, ProviderResource> providerResourceAssembler;

    @Override
    public TeamProviderResource toResource(TeamProvider entity) {
        TeamProviderResource ret = new TeamProviderResource(entity,
                providerResourceAssembler.toResource(entity.getProvider()));
        if (entity.getId() != null) {
            ret.add(linkTo(
                    methodOn(TeamProvidersResource.class).getById(
