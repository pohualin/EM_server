...
public class ReferenceGroupServiceImpl implements ReferenceGroupService {
        ...
        ensureGroupsAndTagsAreValid(groupSaveRequests);
            for (RefGroupSaveRequest request : groupSaveRequests) {
                if (request.getReferenceGroup() != null) {
                    ReferenceGroup savedGroup = save(request.getReferenceGroup());
                    // save the tags pointing to the group
                    List<ReferenceTag> savedTags = saveAllTagsForGroup(request.getReferenceTags(), savedGroup);
    ...
    private List<ReferenceTag> saveAllTagsForGroup(List<ReferenceTag> tags, ReferenceGroup group) {
        List<ReferenceTag> tagsList = new ArrayList<>();
        ...
        return referenceTagPersistence.createAll(tagsList);
    }
